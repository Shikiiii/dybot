...
async def ban(ctx, user: discord.Member, *, reason: str = ""):
	if len(reason) == 0:
		...
		log = discord.Embed(title="{} was banned.".format(user), description="User **{} (ID: {})** was **banned** from the server. \nBanned by: **{} (ID: {})**\nReason: **none, not provided**".format(user, user.id, ctx.message.author, ctx.message.author.id), color=0x000000)
		...
		timestamp=datetime.datetime.now()
		corfor = timestamp.strftime("%d %b, %Y at %H:%M")
		log = discord.Embed(title="{} was banned.".format(user), description="User **{} (ID: {})** was **banned** from the server. \nBanned by: **{} (ID: {})**\nReason: **{}**".format(user, user.id, ctx.message.author, ctx.message.author.id, reason), color=0x000000)
		log.set_footer(text="{}".format(corfor))
		log.set_thumbnail(url=user.avatar_url)
		await logch.send(embed=log)
	await user.ban()
...
async def banid(ctx, id: int, *, reason: str = " "):
	...
	if len(reason) == 0:
		...
		timestamp=datetime.datetime.now()
		corfor = timestamp.strftime("%d %b, %Y at %H:%M")
		log = discord.Embed(title="{} was ID-banned.".format(user), description="User **{} (ID: {})** was **banned** (they probably weren't in the guild). \nBanned by: **{} (ID: {})**\nReason: **none, not provided**".format(user, user.id, ctx.message.author, ctx.message.author.id), color=0x000000)
		log.set_footer(text="{}".format(corfor))
		log.set_thumbnail(url=user.avatar_url)
		await logch.send(embed=log)
	else:
...
async def unban(ctx, id: int, *, reason: str = ""):
	...
	if banEntry is None:
		...
		if banEntry.reason is None:
			...
			timestamp=datetime.datetime.now()
			corfor = timestamp.strftime("%d %b, %Y at %H:%M")
			log = discord.Embed(title="{} was unbanned.".format(user), description="User **{} (ID: {})** was **unbanned**. \nUnbanned by: **{} (ID: {})**\nReason: **none, not provided**".format(user, user.id, ctx.message.author, ctx.message.author.id), color=0x000000)
			log.set_footer(text="{}".format(corfor))
			log.set_thumbnail(url=user.avatar_url)
			await logch.send(embed=log)
		else:
...
async def kick(ctx, user: discord.Member, *, reason: str = ""):
	if len(reason) == 0:
		...
		timestamp=datetime.datetime.now()
		corfor = timestamp.strftime("%d %b, %Y at %H:%M")
		log = discord.Embed(title="{} was kicked.".format(user), description="User **{} (ID: {})** was **kicked**. \nKicked by: **{} (ID: {})**\nReason: **none, not provided**".format(user, user.id, ctx.message.author, ctx.message.author.id), color=0x000000)
		log.set_footer(text="{}".format(corfor))
		log.set_thumbnail(url=user.avatar_url)
		await logch.send(embed=log)
	else:
...
async def mute(ctx, user: discord.Member, *, reason: str = ""):
	...
	if mutedrole in user.roles:
		...
		if len(reason) == 0:
			...
			timestamp=datetime.datetime.now()
			corfor = timestamp.strftime("%d %b, %Y at %H:%M")
			log = discord.Embed(title="{} was muted.".format(user), description="User **{} (ID: {})** was **muted**. \nMuted by: **{} (ID: {})**\nReason: **none, not provided**".format(user, user.id, ctx.message.author, ctx.message.author.id), color=0x000000)
			log.set_footer(text="{}".format(corfor))
			log.set_thumbnail(url=user.avatar_url)
			await logch.send(embed=log)
		else:
...
async def unmute(ctx, user: discord.Member, *, reason: str = ""):
	...
	if mutedrole in user.roles:
		if len(reason) == 0:
			...
			timestamp=datetime.datetime.now()
			corfor = timestamp.strftime("%d %b, %Y at %H:%M")
			log = discord.Embed(title="{} was unmuted.".format(user), description="User **{} (ID: {})** was **unmuted**. \nUnmuted by: **{} (ID: {})**\nReason: **{}**".format(user, user.id, ctx.message.author, ctx.message.author.id, reason), color=0x000000)
			log.set_footer(text="{}".format(corfor))
			log.set_thumbnail(url=user.avatar_url)
			await logch.send(embed=log)
